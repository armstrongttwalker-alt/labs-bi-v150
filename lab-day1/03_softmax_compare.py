...
---------------------
Row-wise softmax: a hand-written Triton kernel vs PyTorch's built-in,
both running on the same BI-V150 GPU.

About the comparison:
- The "torch" column calls torch.softmax, which on BI-V150 with FlagGems
  dispatches to a Triton-based operator.
- The "Triton" column is your kernel, ~30 lines of Python.

Whichever wins, the lesson is the same: Triton is a productive way to
write competitive custom kernels in pure Python.
"""
import time
import torch
import triton
...
def softmax_kernel(
    ...
    # TODO (2): numerically stable softmax
...
def main():
    torch.manual_seed(0)
    print(f"{'Shape':<20} {'torch (ms)':<18} {'Triton (ms)':<14} {'Speedup':<10} {'Max diff':<12}")
    print("-" * 75)
    ...
    print()
    print("Notes:")
    print(" - 'torch' calls torch.softmax, which with FlagGems enabled dispatches")
    print("   to a Triton-based operator.")
    print(" - 'Triton' is your ~30-line Python kernel from this file.")
    print(" - Both run on the SAME GPU; this is a within-GPU comparison.")
