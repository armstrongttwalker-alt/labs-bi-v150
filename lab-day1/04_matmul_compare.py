...
both on the same BI-V150 GPU.

About the comparison:
- 'torch.matmul' on BI-V150 with FlagGems dispatches to optimized Triton
  operators. This is comparable to using cuBLAS on NVIDIA -- a strong baseline.
...
The point is NOT to beat the vendor library. The point is:
  * The same Triton matmul source runs on NVIDIA, AMD, and BI-V150.
...
where we will progressively optimize this matmul.
"""
import time
import torch
import triton
...
def main():
    ...
    print()
    print("Discussion:")
    print(" - torch.matmul uses optimized operators (similar role to cuBLAS).")
    print("   It is a strong baseline.")
    print(" - Our Triton kernel is a straightforward tiled implementation;")
    print("   it does NOT yet exploit several hardware-specific optimizations.")
    print(" - Tomorrow we will progressively tune this kernel.")
