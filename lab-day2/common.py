...
def silent_stdio():
    """Redirect fds 1 and 2 to /dev/null. Use sparingly."""
    sys.stdout.flush()
    sys.stderr.flush()
    saved_out = os.dup(1)
    saved_err = os.dup(2)
...
def make_inputs(M, N, K, dtype=torch.float32, device="cuda", seed=0):
    """Deterministic input tensors on GPU."""
...
# =============================================================================
# Hardware constants (placeholder values for generic GPU)
# =============================================================================
# These are placeholder values. Real values should be measured in step 01.
L0C_CAPACITY_KB = 128     # placeholder: accumulator buffer capacity
L1_CAPACITY_KB = 512      # placeholder: shared memory / L1 capacity
MULTIBUFFER_FACTOR = 2    # typical double-buffering factor
L1_BUDGET_KB = L1_CAPACITY_KB // MULTIBUFFER_FACTOR  # effective budget


def is_legal_tile(BM, BN, BK, dtype_bytes=4):
    """Is (BM, BN, BK) within capacity limits AND aligned?
    Note: This uses placeholder values. Real limits should be measured.
    """
    l0c_kb = BM * BN * dtype_bytes / 1024
    l1_kb = (BM + BN) * BK * dtype_bytes / 1024
    # Alignment requirement (typical for GPUs)
